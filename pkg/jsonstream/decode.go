...
import (
	...
	"k8s.io/kube-openapi/pkg/jsonstream/errors"
)
...
// The provided message must be mutable (e.g., a non-nil pointer to a message).
func (o UnmarshalOptions) Unmarshal(b []byte, m interface{}) error {
...
func (o UnmarshalOptions) unmarshal(b []byte, m interface{}) error {
	...
	if tok.Kind() != json.EOF {
		return dec.unexpectedTokenError(tok, ".", "expected EOF")
	}
...
// func (d decoder) unmarshalMessage(m message, skipTypeURL bool) error {
...
// 		fd := m.messageType.FindField(name)
...
// func (d decoder) unmarshalSingular(m message, fd fieldParser) error {
...
// func (d decoder) unmarshalScalar(fd fieldParser) (protoreflect.Value, error) {
// 	const b32 int = 32
// 	const b64 int = 64
...
func unmarshalInt(tok json.Token, bitSize int) (reflect.Value, bool) {
	switch tok.Kind() {
	case json.Number:
...
func getInt(tok json.Token, bitSize int) (reflect.Value, bool) {
...
func unmarshalUint(tok json.Token, bitSize int) (reflect.Value, bool) {
	switch tok.Kind() {
	case json.Number:
...
func getUint(tok json.Token, bitSize int) (reflect.Value, bool) {
	n, ok := tok.Uint(bitSize)
	if !ok {
		return reflect.Value{}, false
	}
	if bitSize == 32 {
		return reflect.ValueOf(uint32(n)), true
	}
	if bitSize == 16 {
		return reflect.ValueOf(uint16(n)), true
	}
	if bitSize == 8 {
		return reflect.ValueOf(uint8(n)), true
	}
	return reflect.ValueOf(n), true
}

func unmarshalFloat(tok json.Token, bitSize int) (reflect.Value, bool) {
	switch tok.Kind() {
	case json.Number:
	...
	case json.String:
		...
		case "NaN":
			if bitSize == 32 {
				return reflect.ValueOf(float32(math.NaN())), true
			}
			return reflect.ValueOf(math.NaN()), true
		case "Infinity":
			if bitSize == 32 {
				return reflect.ValueOf(float32(math.Inf(+1))), true
			}
			return reflect.ValueOf(math.Inf(+1)), true
		case "-Infinity":
			if bitSize == 32 {
				return reflect.ValueOf(float32(math.Inf(-1))), true
			}
...
func getFloat(tok json.Token, bitSize int) (reflect.Value, bool) {
	n, ok := tok.Float(bitSize)
	if !ok {
		return reflect.Value{}, false
	}
	if bitSize == 32 {
		return reflect.ValueOf(float32(n)), true
	}
	return reflect.ValueOf(n), true
}
